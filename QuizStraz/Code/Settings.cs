...
namespace QuizStraz
{
    ...
    static class Settings
    {
        ...
        static readonly string configXMLFilePath = InitializeConfigPath();
        static string InitializeConfigPath()
        {
            try
            {
                ...
                while (dir != null)
                {
                    if (Directory.Exists(Path.Combine(dir.FullName, "Code")))
                    {
                        ...
                        return Path.Combine(dataDir, "config.xml");
                    }
                    dir = dir.Parent;
                }
            }
            ...
            return Path.Combine(fallback, "config.xml");
        }
        public static async Task HandleConfig(bool returnDefault = false)
        {
            ...
            Config DefaultConfig()
            {
                return new Config
                {
                    ...
                    AlternatingColor2 = ConsoleColor.Yellow,
                };
            }
        }
        ...
        static readonly string achievementsXMLFilePath = InitializeAchievementsPath();
        static string InitializeAchievementsPath()
        {
            try
            {
                string current = Directory.GetCurrentDirectory();
                ...
                while (dir != null)
                {
                    if (Directory.Exists(Path.Combine(dir.FullName, "Code")))
                    {
                        string dataDir = Path.Combine(dir.FullName, "Data");
                        Directory.CreateDirectory(dataDir);
                        return Path.Combine(dataDir, "achievements.xml");
                    }
                    dir = dir.Parent;
                }
            }
            ...
            string fallback = Path.Combine(Directory.GetCurrentDirectory(), "Data");
            Directory.CreateDirectory(fallback);
            return Path.Combine(fallback, "achievements.xml");
        }
